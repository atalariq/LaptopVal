...
<section class="mb-6">
  ...
  <p class="text-slate-500">Skor & verdict transparan berdasarkan spesifikasi dan harga.</p>
</section>

<form method="GET" class="mb-6 flex flex-wrap items-end gap-3">
  <label class="text-sm">Cari
    <input name="q" value={data.filters.search} placeholder="model / brand"
      class="mt-1 block rounded border px-2 py-1" />
  </label>
  <label class="text-sm">Use case
    <select name="use_case" class="mt-1 block rounded border px-2 py-1">
  ...
  <label class="text-sm">Lokasi
    <select name="location" class="mt-1 block rounded border px-2 py-1">
  ...
  <label class="text-sm">Urut
    <select name="sort" class="mt-1 block rounded border px-2 py-1">
  ...
  <button class="rounded bg-slate-900 px-3 py-1.5 text-sm font-medium text-white">Terapkan</button>
</form>

{#if data.items.length === 0}
  <p class="text-slate-500">Tidak ada laptop yang cocok dengan filter.</p>
{:else}
